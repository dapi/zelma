...
| --- | --- | --- | --- | --- | --- |
| `NSM-01` | Session inventory correctness | Главная ценность `zelma` — надежно знать, какие Codex-сессии существуют в `zellij` для текущего репозитория | `unknown` | >= 95% live Codex panes представлены корректными `active` records после `instances detect` | Каждый MVP milestone |
...
| `MET-04` | Registry write safety | Maintainer | `unknown` | 0 corrupted `instances.json` в тестах concurrent/failed writes | Fault-injection tests | CI/local |
...
| `GR-03` | Backward-incompatible registry schema changes | `.zelma/instances.json` должен переживать обновления CLI | Только через documented migration | Добавить versioning/migration или отложить change |
