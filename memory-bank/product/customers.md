...
| --- | --- | --- | --- | --- | --- |
| `SEG-01` | Solo developer using Codex in `zellij` | Вести несколько Codex-сессий в одном репозитории и быстро понимать, какая задача где выполняется | Pane state живет только в голове пользователя и в текущем `zellij` layout | `sessions list` дает полный и понятный инвентарь | Product prompt `2026-07-07` |
...
| --- | --- | --- | --- | --- |
| `ACT-01` | Developer | Запускает `zelma sessions create/list/detect` из shell внутри репозитория | End user / operator | Основной пользователь MVP |
...
- `NA-04` Нельзя молча записывать в `.zelma/sessions.json` неполную активную
