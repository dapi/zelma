...
| --- | --- | --- | --- |
| `SM-01` | `ZelmaSession` | Session Registry | Covers create, detect, stale handling and close/removal semantics |
...
| --- | --- | --- | --- | --- |
| `candidate` | Pane may contain a Codex session, but required identity is incomplete | `sessions detect` sees partial evidence but cannot satisfy active invariants | Evidence resolves to active, or candidate is discarded | no |
| `active` | Registry record represents a live, manageable Codex session in `zellij` | `sessions create` succeeds or `sessions detect` resolves all required refs | Runtime validation fails, user closes/removes session, or migration changes state | no |
| `stale` | Registry record no longer matches live runtime state | Reconciliation/list/detect observes missing pane, missing zellij session or missing Codex runtime | Session reappears/revalidates, user removes, or record is archived | no |
| `closed` | User intentionally ended or removed the managed session record | Explicit future close/remove command or accepted cleanup | None for same record | yes |
...
| --- | --- | --- | --- | --- | --- |
| `TR-01` | none | `active` | `zelma sessions create` | Pane created, Codex launched, all required refs resolved, registry write succeeds | Required refs missing |
| `TR-02` | none | `candidate` | `zelma sessions detect` | Pane has partial Codex evidence but not enough for active record | Pane clearly not running Codex |
...
- `SI-02` `candidate` records must not appear as active sessions in default
