...
| `MET-03` | Safety boundary preservation | Boundary documented upstream | Skill instructions prohibit direct `zellij` calls, direct `.zelma/instances.json` parsing and ungated cleanup confirm | Static review and `rg` checks |
...
- `REQ-04` Preserve the skill boundary: use only `zelma`, not direct `zellij`
  calls, and do not directly read or parse `.zelma/instances.json`.
...
- `REQ-06` Gate destructive stale cleanup: `zelma instances cleanup --confirm
...
- `NS-01` Do not add a second parser for `.zelma/instances.json`.
- `NS-02` Do not call `zellij` directly from the skill.
- `NS-03` Do not introduce a new CLI surface in parallel with `zelma`.
- `NS-04` Do not run or recommend `zelma instances cleanup --confirm --json`
...
- `SC-01` A Codex agent discovers `SKILL.md`; the frontmatter
  `description` matches requests such as "list zelma instances", "create a
  Codex pane with zelma", "detect manual Codex panes", "focus zelma instance 2"
  and "cleanup stale zelma instances".
...
- `SC-03` The skill tells agents to use `zelma` as the only runtime interface
  and not to call `zellij` or parse `.zelma/instances.json` directly.
...
- `NEG-01` A review finds direct `zellij` command guidance or direct
  `.zelma/instances.json` parsing in the skill; the feature must be rejected.
