...
purpose: "Канонический brief для ускорения `sessions detect` и объяснения evidence decisions."
derived_from:
  - ../../product/roadmap.md
...
`sessions detect` может быть медленным и непрозрачным: каждый unresolved
candidate заново сканирует `$CODEX_HOME/sessions`, а пользователь не видит,
почему pane стала `active` или осталась `candidate`.

### Результат

`sessions detect` строит индекс Codex session evidence один раз за запуск и
...
- `REQ-03` Добавить `sessions detect --explain` с evidence verdict/source/reason
  по каждому detected candidate.
- `REQ-04` Поддержать `sessions detect --json --explain` через optional
