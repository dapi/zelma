...
Dashboard и внешние agent UI не должны зависеть от внутренних деталей `.zelma/instances.json` или отдельных CLI команд. Нужен status backend с версионированной моделью, но это не должно блокировать milestone-1.
...
Feature считается готовой только после e2e-теста backend snapshot with active and stale instances.
