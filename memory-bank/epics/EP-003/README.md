---
title: "EP-003: Zellij Read Integration And Detect"
doc_kind: epic
doc_function: index
purpose: "Навигация по draft package epic для zellij introspection и консервативного detect workflow."
derived_from:
  - ../../dna/governance.md
  - brief.md
status: draft
audience: humans_and_agents
---

# EP-003: Zellij Read Integration And Detect

## О разделе

Draft package для read-only zellij integration и `zelma sessions detect`.

## Аннотированный индекс

- [Brief](brief.md)
  Читать, когда нужно понять проблему detect, набросок scope и candidate features.

## Кандидатные Packages Фич

- [FT-010: Zellij Adapter ListSessions](../../features/FT-010/README.md)
- [FT-011: Zellij Adapter ListPanes](../../features/FT-011/README.md)
- [FT-012: Zellij JSON Fixture Tests](../../features/FT-012/README.md)
- [FT-013: Codex Pane Candidate Classifier](../../features/FT-013/README.md)
- [FT-014: Detect Upsert Idempotency](../../features/FT-014/README.md)
