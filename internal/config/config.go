...
const DefaultInstancesListAutoDetectTTL = 5 * time.Second
...
type File struct {
	InstancesList InstancesListConfig `json:"instances_list"`
	StartIssue    StartIssueConfig    `json:"start_issue"`
}

type InstancesListConfig struct {
...
func InstancesListAutoDetectTTL(repoRoot string) (time.Duration, error) {
	cfg, err := Read(repoRoot)
	if err != nil {
		return 0, err
	}
	value := cfg.InstancesList.AutoDetectTTL
	if value == "" {
		return DefaultInstancesListAutoDetectTTL, nil
	}
	ttl, err := time.ParseDuration(value)
	if err != nil {
		return 0, fmt.Errorf("read zelma config %s: invalid instances_list.auto_detect_ttl %q: %w", Path(repoRoot), value, err)
	}
	if ttl <= 0 {
		return 0, fmt.Errorf("read zelma config %s: invalid instances_list.auto_detect_ttl %q: duration must be positive", Path(repoRoot), value)
	}
	return ttl, nil
}
